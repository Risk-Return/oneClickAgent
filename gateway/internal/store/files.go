...
type FileStore struct{ db *DB }

func NewFileStore(db *DB) *FileStore { return &FileStore{db: db} }
...
func (s *FileStore) Create(ctx context.Context, f *model.File) error {
	...
	_, err := s.db.Pool.Exec(ctx,
		`INSERT INTO files (id, user_id, name, size, mime, sha256, status, storage_uri, created_at)
		 VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
		f.ID, f.UserID, f.Name, f.Size, f.Mime, f.SHA256, f.Status, f.StorageURI, f.CreatedAt,
	)
	return err
}
...
func (s *FileStore) GetByID(ctx context.Context, id model.UUID) (*model.File, error) {
	...
	err := s.db.Pool.QueryRow(ctx,
		`SELECT id, user_id, name, size, mime, sha256, status, storage_uri, created_at, purged_at FROM files WHERE id=$1`, id,
	).Scan(&f.ID, &f.UserID, &f.Name, &f.Size, &f.Mime, &f.SHA256, &f.Status, &f.StorageURI, &f.CreatedAt, &f.PurgedAt)
...
func (s *FileStore) UpdateStatus(ctx context.Context, id model.UUID, status model.FileStatus) error {
	_, err := s.db.Pool.Exec(ctx, `UPDATE files SET status=$2 WHERE id=$1`, id, status)
	return err
}

func (s *FileStore) MarkPurged(ctx context.Context, id model.UUID) error {
	now := time.Now().UTC()
	_, err := s.db.Pool.Exec(ctx, `UPDATE files SET status=$2, purged_at=$3 WHERE id=$1`, id, model.FilePurged, now)
	return err
}

func (s *FileStore) UpdateSHA256(ctx context.Context, id model.UUID, sha256 string) error {
	_, err := s.db.Pool.Exec(ctx, `UPDATE files SET sha256=$2 WHERE id=$1`, id, sha256)
	return err
}
...
func (s *FileStore) ListByUser(ctx context.Context, userID model.UUID, cursor *model.UUID, limit int) ([]model.File, *model.UUID, error) {
	...
	var rows pgx.Rows
	var err error
	cols := `id, user_id, name, size, mime, sha256, status, storage_uri, created_at, purged_at`
	if cursor == nil {
		rows, err = s.db.Pool.Query(ctx,
			`SELECT `+cols+` FROM files WHERE user_id=$1 AND status != $2 ORDER BY created_at DESC LIMIT $3`,
			userID, model.FilePurged, limit+1,
		)
	} else {
		rows, err = s.db.Pool.Query(ctx,
			`SELECT `+cols+` FROM files WHERE user_id=$1 AND status != $2 AND created_at < (SELECT created_at FROM files WHERE id=$3) ORDER BY created_at DESC LIMIT $4`,
...
func (s *FileStore) ListStagedCloud(ctx context.Context, olderThan time.Time) ([]model.File, error) {
	rows, err := s.db.Pool.Query(ctx,
		`SELECT id, user_id, name, size, mime, sha256, status, storage_uri, created_at, purged_at
